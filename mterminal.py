...
#
# gedit-mterminal is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU General Public License
# along with gedit-mterminal; if not, write to the Free Software
...
class GeditTerminal(Gtk.HBox):
    ...
    def add_terminal(self, terminalhelper, panel):
        terminalhelper.add_terminal()

    def remove_terminal(self, terminalhelper, panel):
        if len(terminalhelper._terminals) <= 1:
            return
        bottom = terminalhelper.window.get_bottom_panel()  
        bottom.remove_item(panel)
        terminalhelper._terminals.remove(panel)


class MultipleTerminalPlugin(GObject.Object, Gedit.WindowActivatable):
    __gtype_name__ = "MultipleTerminalPlugin"
    WINDOW_DATA_KEY = "MultipleTerminalPluginWindowData"
...
class TerminalWindowHelper(object):
    ...
    def add_terminal(self):
        ...
        bottom.add_item(term, "GeditMultiTerminalPanel-%d" % (term.lastid, ), _("Terminal %d" % (term.lastid, )), image)
    ...
    def on_panel_populate_popup(self, panel, menu):
        ...
        add.connect("activate", lambda menu_item: self.remove_terminal(panel))
        menu.prepend(add)
        
        add = Gtk.MenuItem(_("New Terminal"))
        add.connect("activate", lambda menu_item: self.add_terminal())
        menu.prepend(add)
